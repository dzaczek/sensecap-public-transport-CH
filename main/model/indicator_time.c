...
{
    // Always use Swiss time for this project (Bern/Zurich)
    // CET-1CEST,M3.5.0,M10.5.0/3 handles standard (UTC+1) and daylight saving (UTC+2) automatically
    setenv("TZ", "CET-1CEST,M3.5.0,M10.5.0/3", 1);
    tzset();
    
    // Log the setting
    ESP_LOGI(TAG, "Timezone set to Swiss time (CET-1CEST,M3.5.0,M10.5.0/3)");
}
...
static void __time_cfg_restore(void)
{
    ...
    if( ret == ESP_OK  && len== (sizeof(cfg)) ) {
        ...
        // Default configuration for Switzerland (Bern/Zurich timezone - CET/UTC+1)
        cfg.auto_update = true;
        cfg.auto_update_zone = false;  // Use manual zone for Switzerland
        cfg.daylight = false;  // Not used with smart TZ string
        cfg.time_format_24 = true;
        cfg.zone = 1;  // CET = UTC+1
        cfg.time = 0;
        __time_cfg_set(&cfg);
        
        // Set timezone to Europe/Zurich (same as Bern) - CET-1CEST for winter/summer
